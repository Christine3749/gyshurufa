...
version() { /usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$1/Contents/Info.plist"; }
...
[[ "$status" == *'activation=logout-required'* && "$status" == *"rollback=$backup"* ]] || { echo 'Upgrade status is incomplete.' >&2; exit 1; }
