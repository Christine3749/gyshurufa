...
# A cross-platform release cannot be finalized until the signed/notarized Mac asset
# has updated the same canonical manifest. This also ensures the ZIP embeds final metadata.
if (-not $manifest.macos -or -not [bool]$manifest.macos.signed -or -not [bool]$manifest.macos.notarized -or [string]$manifest.macos.state -notin @('notarized', 'stable')) {
  throw 'Mac package is not signed/notarized in canonical release.json. Do not finalize or publish a mixed release.'
}
...
if ($LASTEXITCODE -ne 0) { throw 'Final release verification failed; latest must not be updated.' }
