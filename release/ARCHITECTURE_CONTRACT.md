...
状态：**已具备 macOS Preview 2.0.8 预发布基线。** 该基线可用于“预发布验收”，但尚未代表全平台 2.0 正式发布。`2.0.8` 仅证明新入口与候选回退策略在 macOS 预览分支落地；它不等于 Windows / Android / iOS / Linux 同步满足。

## 0. 2.0.x 的关键节点（不可降级）

`2.0.x` 不是给当前 Bundle 简单改一个版本号；它是本契约在**各平台独立落地**的发布节点。某平台只有满足以下适用于自己的条件，才可使用 2.0 名称：
...
1. 输入法仍以“黄金上屏链路”优先，输入失败时必须回退。
2. 候选与按键事件可在输入层面稳定工作，且不以未清理的旧会话阻塞。
3. 发布与回退行为可在不破坏主流程前提下验证。

但跨平台桥接（Windows/Android/iOS/Linux）仍未接入 2.0 架构层；因此任何单个平台的 `2.0.x` 仍是该平台预发布基线，不得互相替代。
...
产品的唯一第一优先级是：用户在当前获得焦点的界面中，按键后能持续、正确、低延迟地看到文字上屏。它不是候选 UI、账户、词库或更新功能的从属能力。

```text
物理按键 → Native Bridge → Session Guard → Engine A / LKG B → 宿主应用文本框
                                      └→ 原生简化候选与数字选词
```

- 这条链路离线、无账户、无网络、无动态下载；任何外部组件都不能拥有其唯一控制权。
- Guard 是唯一的提交仲裁者：只接受带匹配会话序号的 Engine 响应；当前组合态在 Engine 或 Panel 崩溃时仍由 Guard 保存。
- Candidate UI 只是渲染层；它丢失、卡住或升级时，原生简化候选、Space、Enter、数字选词和上屏仍必须工作。
- 新功能必须位于黄金链路之外，以默认关闭的能力开关接入；开关异常、配置损坏或 Agent 不启动时，黄金链路保持原样。
- 无法恢复的双 Engine 故障必须明确提示并保留系统中文输入源作为用户逃生路径；不得静默吃键、清空组合态或假装已上屏。
- macOS 不捕获单独 Shift 来切换 GY 模式；中英输入源切换交给系统的 Caps Lock、Fn/Globe 或用户快捷键。不得为了模拟 Windows 热键而改变稳定 Bridge 的事件入口。

绝不承诺硬件、系统或所有磁盘同时故障也不受影响；但承诺**正常更新、UI 改动、词库改动、登录、同步、AI 和实验功能不得令基本文字输入失效**。
...
6. 任一新功能只可在影子模式观察或使用独立 feature flag；它不得替换 Guard、提交仲裁、候选选择或 LKG 路由。
...
- 每次候选、UI、词库、学习或 Agent 变更都必须跑真实上屏回归：连续输入、Backspace、Space、Enter、1–5 选词、简繁、EN、切换焦点和长句；任一失败阻断发布。
- 验收记录必须包含“宿主应用实际文本结果”，不能只包含 trace、控制器激活、进程 PID 或 Engine 自检。
