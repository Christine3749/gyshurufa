...
升级前保留最后一个通过真实键盘验收的安装包和版本记录。候选版本只有通过输入验证后才能被标记为“已知可用”。若失败，产品需要提供：

- **修复 GY 注册：** 仅重建 GY 的输入源记录；若事件入口迁移仍未刷新会话，明确提示保存工作后注销。
- **回退到上一个已知可用版本：** 恢复保存的包、重启当前用户的 GY 服务，并重新验证输入。

不要删除 macOS 全局缓存或影响其他输入法；修复操作必须限定到 GY。回退也不是替代协议稳定性的理由：所有可回退版本仍需保持稳定的 bundle / TIS / connection 身份。
